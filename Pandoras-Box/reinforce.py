"""
The output is so lengthy, make it short & take inspiration from
following code for colab notebook
...
def is_valid_state(state):
    return (0 <= state[0] < grid_size
                and 0 <= state[1] < grid_size
                and state != obstacle_state)
...
for episode in range(episodes):
    ...
    while state != goal_state:
        ...
        q_table[state[0], state[1], actions.index(action)] += alpha * (
            reward + gamma * best_next_action - q_table[
    ...
    print(f"Episode {episode + 1}: Total Reward = {total_reward}", end='\r')
    time.sleep(0.5)
else:
    print()
...
for i in range(grid_size):
    for j in range(grid_size):
        if (i, j) == goal_state:
            ...
            # First letter of the best action
